...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        fileNewAction = self.createAction("&New", self.fileNew,
                QKeySequence.New, "filenew", "Get the file from VHD")
        ...
        self.updateWindowMenu()
        self.setWindowTitle("VHD Inspector")
    ...
    def closeEvent(self, event):
        ...
        if failures and \
           QMessageBox.warning(self, "VHD Inspector -- Save Error",
                    "Failed to save%s\nQuit anyway?" % \
                    "\n\t".join(failures),
    ...
    def fileNew(self):
        dialog = newconnectdlg.NewConnectDlg(self)
        if dialog.exec_():
            self.url = unicode(dialog.urlLineEdit.text())
            self.accountkey = unicode(dialog.accountkeyLineEdit.text())
            self.filename = unicode(dialog.filenameLineEdit.text())
            self.path = unicode(dialog.pathLineEdit.text())
            self.extension = unicode(dialog.extensionLineEdit.text())
            self.type = unicode(dialog.typeLineEdit.text())
            cmd = "python inspector.py"
            options = list()
            if self.url:
                options.append("-u " + self.url)
            if self.accountkey:
                options.append("-k " + self.accountkey)
            if self.filename:
                options.append("-f " + self.filename)
            if self.path:
                options.append("-p " + self.path)
            if self.extension:
                options.append("-e " + self.extension)
            if self.type:
                options.append("-t " + self.type)
            cmd += " " + " ".join(options)
            cmd += " >stdouterr 2>&1"
            try:
                os.system(cmd)
                ...
                QMessageBox.warning(self, "VHD Inspector",
                        "Failed to execute %s: %s" % (cmd, e))
            filename = os.path.join(self.path, self.filename)
            filename = QString(self.url.split('/')[-1] + filename)
    ...
    def loadFile(self, filename):
        ...
        try:
            ...
            QMessageBox.warning(self, "VHD Inspector -- Load Error",
    ...
    def fileSaveAll(self):
        ...
        if errors:
            QMessageBox.warning(self, "VHD Inspector -- Save All Error",
                    "Failed to save\n%s" % "\n".join(errors))
